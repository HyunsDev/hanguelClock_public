...
# 기본 내용 설정 (언어 종속)
!define TXT_NAME_K  			"지금몇시계"					; 프로그램 이름
!define TXT_PUBLISHER_K			"Hyuns현스"					; 프로그램 추가/제거에 보일 publisher 이름
!define TXT_LNKNAME_K			"지금몇시계"					; 프로그램 그룹의 링크 이름
!define TXT_PROGRAM_GROUP_NAME_K	"Hyuns현스"					; 프로그램 그룹 이름
!define TXT_BRANDING_K			"지금몇시계와 함께하기"				; 하단 branding text
!defin TXT_COPANY_NAME_K		"Hyuns 현스"					; 인스톨러에 표시될 회사 이름
!define TXT_UNINST_DISPNAME_K		"지금몇시계"					; 언인스톨러에 표시될 이름
!define TXT_UNINST_LNKNAME_K		"지금몇시계를 떠나보내기"				; 언인스톨러 링크 이름
!define TXT_HOMEPAGE_URL_K		"http://www.kipple.pe.kr"		; 홈페이지 주소
!define TXT_LICENSE_FILENAME_K		"_license.rtf"				; 라이센스 파일이름                               
!define TXT_VI_COMPANY_NAME_K		"Hyuns"					; 버전정보 - 회사명
!define TXT_VI_COPYRIGHT_K		"copyright(c) Hyuns"			;         - 저작권 정보
!define TXT_PROGRAM_GROUP_PARENT_K	"Hyuns현스"					; 프로그램 그룹 이름
...
!define	APP_INSTDIR			"What Clock is it"				; Program Files 에 설치될 폴더 이름.
;!define APP_COMPANYDIR			"KIPPLE"				; Program Files\COMPANY NAME\PROGRAM NAME 에 설치될 경우  - 안쓸경우 NULL string("") 대신 주석처리하도록 한다.
!define	APP_REGPOS			"SOFTWARE\WhatClockIsIt"		; 설치 폴더가 저장될 위치, 프로그램이 HKCU 에 데이타 저장하는 레지스트리 - 언인스톨시 삭제될 레지스트리 위치
!define	APP_OUTNAME			"WhatClockIsItSETUP.EXE"			; setup 파일명
!define	APP_CLASSNAME			"Whatclockisit"				; app class name
!define	APP_EXENAME			"whatclockisit.EXE"				; 설치 완료후 실행될 실행 파일 이름
